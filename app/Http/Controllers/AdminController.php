...
class AdminController extends Controller
{
    ...
    public function addGalleryItems(Request $request){
        try{
            $validation = [
                GalleryItem::LOCAL_IMAGE=>"bail|nullable|required_without_all:image_link|array",
        ...
        $data = GalleryItem::select(
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                           ...
                           $btn = '<a data-row="'.base64_encode(json_encode($row)).'" href="javascript:void(0)" class="edit btn btn-primary btn-sm mt-2">Edit</a>'.
                           '<a href="javascript:void(0)" onclick="deleteGallery(\''.$row->{GalleryItem::ID}.'\')" class="edit btn btn-danger btn-sm mt-2">Delete</a>';
